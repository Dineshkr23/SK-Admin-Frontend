...
function Arch_Engi() {
  ...
  const handleOfficePincodeLookup = async (rawPincode: string) => {
    ...
    try {
      ...
      if (cityTownValue) {
        setFormInputValue("currentCityTown", cityTownValue);
      }
      if (districtValue) {
        setFormInputValue("currentDistrict", districtValue);
      }
      if (talukValue) {
        setFormInputValue("currentTaluk", talukValue);
      }
      if (landmarkValue) {
        setFormInputValue("currentLandmark", landmarkValue);
      }
  ...
  const validateFormBeforeSubmit = (formData: FormData): string => {
    const requiredTextFields: Array<{ key: string; label: string }> = [
      ...
      { key: "city", label: "City" },
      { key: "currentAddress1", label: "Office Address 1" },
      { key: "currentDistrict", label: "Office District" },
      { key: "currentTaluk", label: "Office Taluk" },
      { key: "currentCityTown", label: "Office City/Town" },
      { key: "currentPincode", label: "Office Pincode" },
      ...
      { key: "dealerPlace", label: "Dealer Place" },
    ];
    ...
    const officePincode = getFieldValue(formData, "currentPincode");
    if (!isValidIndianPincode(officePincode)) {
      return "Office Pincode must be 6 digits.";
    }

    const dateOfBirthValue = getFieldValue(formData, "dateOfBirth");
    if (!parseIsoDate(dateOfBirthValue)) {
        ...
        <section style={sectionStyle}>
          ...
          <form ref={formRef} onSubmit={handleMasonSubmit}>
            ...
            <div style={responsiveUploadGrid}>
              <UploadInputCard
                title="Photograph*"
                subtitle="Please Capture your photograph here"
